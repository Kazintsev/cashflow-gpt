...
> ID: application_api · Для: developer, runtime_agent · Тип: observed_snapshot
>
> Прикладные маршруты и SQL-примеры. [Карта документации](index.md) · [Манифест](manifest.json)
...
| `previous_day_operations` | Дата, часовой пояс, границы, количество и все операции предыдущего дня |
...
Добавление от 11 сентября 2026: `previous_day_operations` — блок сверки операций за календарный день перед `business_date`. Это совместимое расширение v3; прежние ключи и расчёты сохранены. В старой установке поле может отсутствовать: это «список недоступен», а не пустой день.

| Поле блока | Значение |
|---|---|
| `date`, `timezone` | Дата сверки и бизнес-часовой пояс из настроек |
| `start_at`, `end_at_exclusive` | Вчера 00:00 включительно; сегодня 00:00 исключительно |
| `count` | Полное количество операций |
| `operations` | Полный массив, без лимита; порядок `occurred_at`, затем `transaction_id` |

Элемент массива: `transaction_id`, `occurred_at`, `local_time` (HH:MM:SS), `transaction_type`, `amount`, `currency`, `description`, `budget_effect`, `from_account_id`, `from_account_name`, `to_account_id`, `to_account_name`, `liability_id`. Неизвестная сторона операции — null; это не нулевая сумма. `amount` положителен, направление задаётся типом и счетами.

Выборка использует фактический `occurred_at`, а не время создания записи. Включены все типы, бюджетные признаки и счета, в том числе неактивные. Одна транзакция — одна строка: строки чека, категории и распределения не размножают её. Перевод показывать «откуда → куда», не считать ещё одним расходом. Снимки балансов и плановые события не являются операциями и в список не входят.

В конце полного статуса вывести «Операции за вчера — ДД.ММ.ГГГГ» и таблицу `ID | Время | Тип | Описание | Счёт / направление | Сумма`, затем количество. Показать все строки. При успешном ответе с `count=0` и `operations=[]` написать «За этот день операций в базе нет». Не утверждать, что все реальные покупки учтены: это список записей для сверки пользователем, а не сверка с банком. Исторический вызов отражает текущую редакцию операций, без восстановления удалённых или прежних версий.

<a id="payments-read"></a>
